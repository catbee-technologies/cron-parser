...
export class CronExpression {
  ...
  #matchHour(currentDate: CronDate, dateMathVerb: DateMathOp, reverse: boolean): boolean {
    ...
    // On DST transition days, preserve dstStart until we're sure we've processed the transition.
    const isDstTransitionDay = this.#checkDstTransition(currentDate);
    ...
    if (nextHour === null) {
      if (!isDstTransitionDay) {
        ...
        // On DST day, skip to next day but reset dstStart since we're moving to next day
        currentDate.dstStart = null;
        currentDate.applyDateOperation(dateMathVerb, TimeUnit.Day, hours.length);
      }
      return false;
